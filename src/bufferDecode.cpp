...
dataLevelProcess level_process;
#endif
...
static unsigned int last_azimuthAngle = 0.0;
...
inline int checkOneFrame(unsigned int last_value, unsigned int cur_value,sensor_msgs::PointCloud2 &outCloud){
    int rtn = 0 ;
    if (s_findStartZero == false) {
        if (cur_value>=0 && cur_value<=80 ) {
            s_lineCount = 0 ;
            s_lineData.clear();
            s_findStartZero = true;
        }
...
//ROS_INFO("s_angleSum=%f",s_angleSum);
  if(s_angleSum >= s_angle_duration) {
//       timeval tval_begin;
//      gettimeofday(&tval_begin, NULL);
...
//      s_angleStep = (tmpDif) / 32.0;
    for(int i =0; i < GROUP_NUM_ORI; i++)
    {
        ...
        if(i <=4)
        {
            ...
            if(DEVICE_ID==0x4F||DEVICE_ID==0x55||DEVICE_ID==0x56||DEVICE_ID==0x57||DEVICE_ID==0x58||DEVICE_ID==0x5C)
            {
...
//            tmpAngle = (mtBlock->azimuth_angle+j*s_angleStep)*0.01;
            ...
            pulse_width = ((mtBlock->points[j].intensity_pulse[1] & 0x0F) << 8)| mtBlock->points[j].intensity_pulse[2];
            tmpPulseWidth = pulse_width * CONVERT_4mm_2m;
            tmpXyz.intent = (mtBlock->points[j].intensity_pulse[0]<<4)|((mtBlock->points[j].intensity_pulse[1]>>4)&0x0f);
            tmpXyz.laserid = j;
            tmpXyz.range = tmpRange;
            tmpXyz.pulseWidth = tmpPulseWidth;
            tmpXyz.mirrorid =0;
#ifdef USE_PARALLEL_ALGORITHM
            level_process.data_level_convert(&tmpXyz);
#endif
            calcXyz(DEVICE_ID, tmpXyz.range,tmpAngle,tmpXyz);
            if(g_xyzFile) {
...
//        }
        ...
        else if(DEVICE_ID == 0x5B || DEVICE_ID == 0x5C || DEVICE_ID == 0x24)
        {
            angular_z = tmpAngleDiff*UINTCONVERT/(32*3.33);
        }
        else
        ...
        for(int j=0; j< POINT_NUM_USER*2; j+=2)//16
        {
			...
			case RFANS_PRODUCT_MODEL_V6K_16M_0X24:
                ...
                if(j < 15)
                {
                    tmpXyz1.timeflag = packet->gps_timestamp + timeOffset*32*(i/echo_flag)+DELTA_T_V6K_16M_0x24[j];
                    tmpXyz2.timeflag = packet->gps_timestamp + timeOffset*32*(i/echo_flag)+DELTA_T_V6K_16M_0x24[j+1];
                    tmpAngle1 = mtBlock->azimuth_angle*UINTCONVERT+angular_z*(DELTA_T_V6K_16M_0x24[j]);
                    tmpAngle2 = mtBlock->azimuth_angle*UINTCONVERT+angular_z*(DELTA_T_V6K_16M_0x24[j+1]);

                }
                else{

                    tmpXyz1.timeflag = packet->gps_timestamp+timeOffset*32*(i/echo_flag)+DELTA_T_V6K_16M_0x24[j-16]+53.28;
                    tmpXyz2.timeflag = packet->gps_timestamp+timeOffset*32*(i/echo_flag)+DELTA_T_V6K_16M_0x24[j-15]+53.28;
                    tmpAngle1 = mtBlock->azimuth_angle*UINTCONVERT+angular_z*(DELTA_T_V6K_16M_0x24[j-16]+53.28);
                    tmpAngle2 = mtBlock->azimuth_angle*UINTCONVERT+angular_z*(DELTA_T_V6K_16M_0x24[j-15]+53.28);
                }
                break;
            case RFANS_PRODUCT_MODEL_V6K_16G_0x5C:
                timeOffset = 3.33;
            ...
            case RFANS_PRODUCT_MODEL_V6K_32M_0X5A:
              case RFANS_PRODUCT_MODEL_V6K_32MP:
                timeOffset = 1.5625;
                if(echo_flag == 2) {
            ...
            case CFANS_128_V2:
                timeOffset = 1.5625;
                tmpXyz1.timeflag = packet->gps_timestamp + timeOffset*32*(i/echo_flag) + DELTA_T_CFANS_0x80[j];
                tmpXyz2.timeflag = packet->gps_timestamp + timeOffset*32*(i/echo_flag) + DELTA_T_CFANS_0x80[j+1];
                tmpAngle1 = mtBlock->azimuth_angle*UINTCONVERT + angular_z*DELTA_T_CFANS_0x80[j];
                tmpAngle2 = mtBlock->azimuth_angle*UINTCONVERT + angular_z*DELTA_T_CFANS_0x80[j+1];
                break;
            case CFNAS_32_0x81:
                timeOffset = 6.25;
                tmpXyz1.timeflag = packet->gps_timestamp + timeOffset*(j+(i/echo_flag)*POINT_NUM_USER*2);
                tmpXyz2.timeflag = packet->gps_timestamp + timeOffset*(j+1+(i/echo_flag)*POINT_NUM_USER*2);
            ...
            } else if(CFANS_128_V2 == DEVICE_ID){
                calcCFansCoor(tmpRange1,tmpAngle1,mirrorID,tmpXyz1.laserid,tmpXyz1);
                calcCFansCoor(tmpRange2,tmpAngle2,mirrorID,tmpXyz2.laserid,tmpXyz2);
            } else {
...
inline int processPacketUserSimple(PACKET_USER_SIMPLE_S* packet, sensor_msgs::PointCloud2 &outCloud)
{
    ...
    for(int i =0; i < GROUP_NUM_USER_SIMPLE; i++)
    {
        ...
        else if(DEVICE_ID==0x5C ||DEVICE_ID==0x5B || DEVICE_ID == 0x24){
        ...
        for(int j=0; j< POINT_NUM_USER_SIMPLE; j++)
        {
            ...
            azimuth_angle = (mtBlock->azimuth_angle)%36000;
            //ROS_INFO("azimuth_angle=%d,device_ID=%x",mtBlock->azimuth_angle,DEVICE_ID);
            switch (DEVICE_ID) {
            case RFANS_PRODUCT_MODEL_V6A_X16M_0X4F:
            case RFANS_PRODUCT_MODEL_V6A_E1_0X55:
            case RFANS_PRODUCT_MODEL_V6A_E2_0X56:
			...
			case RFANS_PRODUCT_MODEL_V6K_16M_0X24:
                ...
                if(j<=15)
                {
					tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*(i/echo_flag)+DELTA_T_V6K_16M_0x24[j];
                    tmpAngle = mtBlock->azimuth_angle*UINTCONVERT+angular_z*DELTA_T_V6K_16M_0x24[j];
                }
                else{
					tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*(i/echo_flag)+DELTA_T_V6K_16M_0x24[j-16]+53.28;
                    tmpAngle = mtBlock->azimuth_angle*UINTCONVERT+angular_z*(DELTA_T_V6K_16M_0x24[j-16]+53.28);
                }
                break;
            case RFANS_PRODUCT_MODEL_V6G_X32_0X33:
            case RFANS_PRODUCT_MODEL_V6B_X32_0X50:
            ...
            case RFANS_PRODUCT_MODEL_V6K_32MP:
                timeOffset = 1.5625;
                if(echo_flag == 2){
                    tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*(i/2)+DELTA_T_V6K_32M_0x60[j];
                }
                else{
                    tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*i+DELTA_T_V6K_32M_0x60[j];
                }
                tmpAngle = mtBlock->azimuth_angle*UINTCONVERT+angular_z*DELTA_T_V6K_32M_0x60[j];
                break;

            case RFANS_PRODUCT_MODEL_V6K_32:
                timeOffset = 1.5625;
                if(echo_flag == 2){
                    tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*(i/2)+DELTA_T_V6K_32_0x5E[j];
                }
                else{
                    tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*i+DELTA_T_V6K_32_0x5E[j];
                }
                tmpAngle = mtBlock->azimuth_angle*UINTCONVERT+angular_z*DELTA_T_V6K_32_0x5E[j];
                break;
                //2020.06.30 LICHAO
            case RFANS_PRODUCT_MODEL_V6K_32P:
                timeOffset = 1.5625;
                if(echo_flag == 2){
                    tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*(i/2)+DELTA_T_V6K_32P_0x5F[j];
                }
                else{
                    tmpXyz.timeflag = packet->gps_timestamp+timeOffset*32*i+DELTA_T_V6K_32P_0x5F[j];
                }
                tmpAngle = mtBlock->azimuth_angle*UINTCONVERT+angular_z*DELTA_T_V6K_32P_0x5F[j];
                break;
            ...
            case CFANS_128_V2:
                timeOffset = 1.5625;
                tmpXyz.timeflag = packet->gps_timestamp + timeOffset*32*(i/echo_flag) + DELTA_T_CFANS_0x80[j];
                tmpAngle = mtBlock->azimuth_angle*UINTCONVERT + angular_z * DELTA_T_CFANS_0x80[j];
                break;
            case CFNAS_32_0x81:
                timeOffset = 6.25;
                ...
                tmpXyz.timeflag = packet->gps_timestamp +
                        timeOffset * (j + (i/echo_flag) * POINT_NUM_USER_SIMPLE);
                break;

            default:
              ROS_INFO(" no device_id");
                break;
            }
            ...
            } else if (DEVICE_ID == CFANS_128_V2){
                calcCFansCoor(tmpRange,tmpAngle,mirrorID, tmpXyz.laserid,tmpXyz);
            }
            else {
                calcXyz(DEVICE_ID,tmpRange,tmpAngle,tmpXyz);
            }
            ...
            ++s_lineCount;
            if(s_lineCount>=LINE_POINT_COUNT) s_lineCount = LINE_POINT_COUNT-1;
            last_azimuthAngle = azimuth_angle;

        }
    }

    //ROS_INFO("processPacketUserSimple:rtn=%d",rtn);
    return rtn;
        ...
        for(int i=0 ; i< inPack.udpCount; i++)
        {
            ...
            else if(data_level_ ==3)
            {
                ...
                if(processPacketUserSimple(tmpFrameUSERSimple,outCloud))
                {
                  ...
                  //ROS_INFO("rosOut.publish(outCloud)");
                  rosOut.publish(outCloud);
                    SSBufferDec::ResetPointCloud2(outCloud);
                }
            }
...
void SSBufferDec::InitPointcloud2(sensor_msgs::PointCloud2 &initCloud) {
    ...
    initCloud.fields.resize(10);          //line format
